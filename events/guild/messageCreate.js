...
module.exports = async (client, message) => {
  ...
  if (!cmd || cmd.length == 0) {
    if (mPrefix.includes(client.user.id)) {
      message.reply({
        embeds: [new Discord.MessageEmbed().setColor(ee.color).setFooter(ee.footertext, ee.footericon).setDescription(`<a:pikachu:938864125977714709> | Olá! Meu prefix é \`${prefix}\`\n\nUse \`${prefix}help\` para mais informações.`)]
      })
    }
    return;
  }
      ...
      if (!botchannels.includes(message.channel.id) && !message.member.permissions.has("ADMINISTRATOR")) {
        return message.reply({
          embeds: [new Discord.MessageEmbed()
            ...
            .setTitle(`<:errado:938638337072300103> | Você não tem permissão para usar este comando aqui!`)
            .setDescription(`Por favor, faça isso em um desses:\n> ${botchannels.map(c=>`<#${c}>`).join(", ")}`)
          ]
        })
      }
    }
      ...
      if (command.minargs && command.minargs > 0 && args.length < command.minargs) {
        return message.reply({
          embeds: [new Discord.MessageEmbed()
            .setColor(ee.wrongcolor)
            .setFooter(ee.footertext, ee.footericon)
            .setTitle(`<:errado:938638337072300103> | Uso incorreto do comando!`)
            .setDescription(command.argsmissing_message && command.argsmissing_message.trim().length > 0 ? command.argsmissing_message : command.usage ? `Use: ` + command.usage : `Uso incorreto do comando`)
          ]
        }).then(msg => {
          setTimeout(() => {
            msg.delete().catch((e) => {
      ...
      if (command.maxargs && command.maxargs > 0 && args.length > command.maxargs) {
        return message.reply({
          embeds: [new Discord.MessageEmbed()
            ...
            .setFooter(ee.footertext, ee.footericon)
            .setTitle(`<:errado:938638337072300103> | Uso incorreto do comando!`)
            .setDescription(command.argstoomany_message && command.argstoomany_message.trim().length > 0 ? command.argstoomany_message : command.usage ? `Use: ` + command.usage : `Uso incorreto do comando`)
          ]
        }).then(msg => {
          setTimeout(() => {
            msg.delete().catch((e) => {
      ...
      if (command.maxplusargs && command.maxplusargs > 0 && args.join(` `).split(`++`).filter(Boolean).length > command.maxplusargs) {
        return message.reply({
          embeds: [new Discord.MessageEmbed()
            ...
            .setFooter(ee.footertext, ee.footericon)
            .setTitle(`<:errado:938638337072300103> | Uso incorreto do comando!`)
            .setDescription(command.argstoomany_message && command.argstoomany_message.trim().length > 0 ? command.argsmissing_message : command.usage ? `Use: ` + command.usage : `Uso incorreto do comando`)
          ]
        }).then(msg => {
          setTimeout(() => {
            msg.delete().catch((e) => {
