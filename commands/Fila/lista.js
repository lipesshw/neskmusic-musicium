...
module.exports = {
	...
	description: "Mostre a fila de músicas que irão tocar.", //the command description for Slash Command Overview
	cooldown: 10,
			...
			try {
				...
				for (let i = 0; i < theSongs.length; i += 10) {
					...
					const embed = new MessageEmbed()
						...
						.setDescription(`${info}`)
					if (i < 10) {
						embed.setTitle(`📑 | Fila de música deste servidor.`)
				...
				const Menu = new MessageSelectMenu()
					...
					.setPlaceholder("Selecionar uma página")
					.addOptions([
						pages.map((page, index) => {
							let Obj = {};
							Obj.label = `Página ${index}`
							Obj.value = `${index}`;
							Obj.description = `Visualize a página ${index}/${pages.length - 1}`
							return Obj;
						})
					])
