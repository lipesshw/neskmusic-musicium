		...
		try {
			...
			try {
				...
				if (!args[0]) {
					return message.reply({
						embeds: [new MessageEmbed()
							.setColor(ee.wrongcolor)
							.setFooter(ee.footertext, ee.footericon)
							.setTitle(`<:errado:938638337072300103> | Por favor, adicione um volume`)
							.setDescription(`**Use:**\n> \`${client.settings.get(message.guild.id, "prefix")}volume <novo volume | 0-150>\``)
						],
					});
				}
				...
				if (volume > 150 || volume < 0) return message.reply({
					embeds: [
						new MessageEmbed().setColor(ee.wrongcolor).setDescription(`<:errado:938638337072300103>| O Volume deve estar entre \`0\` e \`150\`!`)
					],
				...
				message.reply({
					embeds: [new MessageEmbed()
					  .setColor(ee.color)
					  .setTimestamp()
					  .setTitle(`<:musica2:938638011162296390> | \`${member.user.tag}\` alterou o volume para \`${volume}\`!`)]
				})
			} catch (e) {
